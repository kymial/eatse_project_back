...
function SignIn({ onLinkClickHandler } : Props) {
    ...
    const signInResponse = (result: SignInResponseDto | ResponseDto | null) => {
        ...
        const message = 
            ...
            result.code === 'SF' ? '로그인 정보가 일치 하지 않습니다.' :
...
const idCheckResponse = (result: ResponseDto | null) => {
    ...
    const idMessage =
        !result ? '서버가 문제가 있습니다.' :
        result.code === 'VF' ? '아이디는 빈 값 혹은 공배긍로만 이루어질 수 없습니다.' :
...
const emailAuthResponse = (result: ResponseDto | null) => {
    ...
    const emailMessage = !result ? '서버가 문제가 있습니다.' :
...
const signUpResponse = (result: ResponseDto | null) => {
    ...
    const onPasswordChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const passwordMessage = isPasswordPattern ? '' : value ? '영문, 숫자를 혼용하여 8 ~ 13자를 입력하여주세요' : '';
    ...
    const onPasswordCheckChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const passwordCheckMessage = isEqualPassword ? '' : value ? '비밀번호가 일치하지 않았습니다.' : '';
